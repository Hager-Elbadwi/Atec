            ...
            <div class="portlet-body">
                ...
                <div class="input-medium margin-top-10">
                    <select name="role" class="form-control">
                      ...
                      @foreach ($roles as $key => $value)
                        <option value="{{ $key }}" {{ count($admin->roles) && $admin->roles[0]->id == $key ? 'selected' : '' }}>{{ $value }}</option>
                      @endforeach
                    </select>
                </div>
              </div>
              <div class="mt-radio-inline">
                  <label class="mt-radio">
                      <input type="radio" name="status" value="1" {{ $admin->status == 1 ? 'checked' : '' }}> Active
                      <span></span>
                  </label>
                  <label class="mt-radio">
                      <input type="radio" name="status" value="0" {{ $admin->status == 0 ? 'checked' : '' }}> Not Active
                      <span></span>
                  </label>
              </div>
